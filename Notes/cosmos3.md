...
3. For actions, as it only appears in mid-training, it focuses on four physical-AI pillars: egocentric motion (67% data), robotics (16%), autonomous vehicles (8%), and camera motion (7%). "For data with multiple synchronized viewpoints, we concatenate the views into a canvas and store the camera layout in metadata".
...
The reasoner is trained first. A pre-trained ViT, multimodal projectors, and a pre-trained LLM. Unfreeze all and train all components jointly from the start of pre-training. Trained in a next token prediction manner and for two epochs over the full pre-training mixture using a no-replacement sampler that
uniformly concatenates all datasets. Restrict training to sequences of at most 16k tokens, with per-sample limits of 2048 image tokens and 8192 video tokens, for efficiency. Square-root normalized per-token loss
weighting to balance the contributions of short and long sequences. Optimization uses AdamW with a peak learning rate of 5×10−5 for the language model and projector, and
5×10−6 for the ViT. All learning rates follow a cosine decay schedule to 0.1× of the peak value after a 10% linear warm-up phase. We use Adam coefficients (𝛽1, 𝛽2) = (0.9, 0.999). Training additionally uses weight decay of 0.05 and gradient clipping with a global norm threshold of 1.0.

### reasoner SFT
Reasoner SFT training used an importance-based sampling dataloader depending on the dataset's importance, quality, and scale. pre-training dataset also retained and with 1:4 pre-training-to-SFT sampling-budget ratio for robustness. Also it includes a lightweight instruction-following dataset. 
...
### genrator pre-training
The generator is optimized under a rectified flow matching objective across all modalities. Three resolutions (256p, 480p, 720p) are covered in the training, and different sequence lengths are packed into batches for maximum GPT utilization. Only the generation-specific parameters are updated during the generator pre-training. The reasoner tower remains frozen, preserving the language and visual understanding capabilities.

### generator mid-training
It is similar to pre-traing with higher quality and more focused data. Two more modalities, actions and audio generation. 
